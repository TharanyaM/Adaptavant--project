...
public class TodoListServlet {
	...
	public String TodoList(ModelMap model, HttpServletRequest req, HttpServletResponse res, HttpSession session) throws java.text.ParseException {
		...
		if(session.getAttribute("user")!=null)
		{
			...
			String qdate = req.getParameter("date").trim().toString();
			System.out.println(qdate);
			q1.setFilter("userId == sessionUserId && groupId == formGroupId && date == qdate");
			q1.declareParameters("long sessionUserId, long formGroupId, String qdate"); 
			...
			try {
				...
				entryResults = (List<Entry>)q1.execute(sessionUserId, formGroupId, qdate);
				groupResults = (List<Group>)q2.execute(((User)session.getAttribute("user")).getId());
			...
			setUserDate(format.parse(qdate));
			model.addAttribute("userDate", userDate);
